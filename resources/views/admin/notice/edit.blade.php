...
<div class="p-4">
    ...
    <form action="{{route('notice.update',$notice->id)}}" method="POST" class="mt-5">
        @csrf
        <input type="text" class="w-full rounded-lg mt-2" name="notice" placeholder="Enter Notice" value="{{$notice->notice}}">
        @error('notice')
            <span class="text-red-600 -mt-4">* {{$message}}</span>
        @enderror
        
        <input type="text" class="w-full rounded-lg mt-2" name="priority" placeholder="Enter Priority" value="{{$notice->priority}}">
        @error('priority')
            <span class="text-red-600 -mt-4">* {{$message}}</span>
        @enderror
